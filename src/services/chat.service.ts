...
export const chatService = {
  ...
  getMyConversations: async (): Promise<ConversationResponse[]> => {
    const { data } = await axiosClient.get('/api/v1/chat/conversations');
    return data;
  },
  ...
  getOrCreateConversation: async (petitionId: number, providerId: number): Promise<ConversationResponse> => {
    const { data } = await axiosClient.post('/api/v1/chat/conversations', {
      petitionId,
      providerId
    });
    return data;
  ...
  getMessages: async (conversationId: number): Promise<MessageResponse[]> => {
    const { data } = await axiosClient.get(`/api/v1/chat/conversations/${conversationId}/messages`);
    return data;
  },

  // Envía un mensaje nuevo
  sendMessage: async (conversationId: number, content: string): Promise<MessageResponse> => {
    const { data } = await axiosClient.post(`/api/v1/chat/conversations/${conversationId}/messages`, {
      content
    });
    return data;
  }
